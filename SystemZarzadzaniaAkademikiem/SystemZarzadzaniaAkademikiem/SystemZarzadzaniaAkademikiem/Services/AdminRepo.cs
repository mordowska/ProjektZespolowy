using SQLite;
using SQLite.Net.Cipher.Interfaces;
using System;
using System.Collections.Generic;
using System.Text;
using System.Threading.Tasks;
using SystemZarzadzaniaAkademikiem.Data;
using SystemZarzadzaniaAkademikiem.Models;

namespace SystemZarzadzaniaAkademikiem.Services
{
    public class AdminRepo : IAdminRepo
    {
        ISecureDatabase database;
        #region
        static string keySeed = "";
        #endregion
        public AdminRepo(AppDatabase database)
        {
            this.database = database;
            if (this.database.SecureGet<Admin>("1",keySeed)==null)
            {
                this.database.SecureInsert(new Admin { Id="1",Login="Admin",Password="S3cr3tP@ss"},keySeed);
            }
        }
        public Admin GetAdmin()
        {
            return database.SecureGet<Admin>("1",keySeed);
        }
        public int UpdateAdmin(Admin admin)
        {
            return database.SecureUpdate(admin,keySeed);
        }
    }
}
